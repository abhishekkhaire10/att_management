...
def index():
    if request.method == 'POST':
        ...
        if account_type == 'Professor':
            ...
            cur.execute('SELECT id_teacher, name, password FROM teacher WHERE name = "'+professor_name+'"AND password = "'+professor_password+'"')
            result = cur.fetchall()
            ...
            cur = mysql.connection.cursor()
            cur.execute('SELECT id_student, name, password FROM student WHERE name = "'+student_name+'"AND password = "'+student_password+'"')
            result = cur.fetchall()
...
def admin_homepage():
    if 'professor_name' in session:
        ...
        cur = mysql.connection.cursor()
        cur.execute('SELECT course FROM att_tracker WHERE id_teacher = "'+str(professor_id)+'"AND teacher_name = "'+professor_name+'"AND status = "open"')
...
def student_homepage():
    if 'student_name' in session:
        ...
        for course in course_list:
            cur = mysql.connection.cursor()
            cur.execute('SELECT COUNT(*) FROM '+course+' WHERE student_id = "'+str(student_id)+'"')
            result = cur.fetchall()
...
def mark_attendance():
    if request.method == 'POST':
        ...
        cur = mysql.connection.cursor()
        status = 'open'
        cur.execute('SELECT * FROM att_tracker WHERE status = "'+status+'"AND course = "'+subject+'"')
        results = cur.fetchall()
        print(len(results))
        print(results)
        if len(results) > 0:
            ...
            print(student_id)
            q1 ='INSERT INTO '+subject_name_underscored+ '(student_id, student_name, marking_time, marking_date)' 
            q2 = 'VALUES("'+str(student_id)+'","'+name+'", now(), CURDATE())'
            cur.execute(q1+q2)
            mysql.connection.commit()
            cur.close()
            flash("Attendance marked for {} successfully".format(subject.replace('_',' ').upper()),category='success')
            return redirect(url_for('student_homepage'))
        else:
...
def turn_on_attendance():
    if request.method == 'POST':
        ...
        status = 'open'
        cur = mysql.connection.cursor()
        cur.execute('SELECT * FROM att_tracker WHERE id_teacher = "'+str(professor_id)+'"AND teacher_name = "'+professor_name+'"AND status ="'+status+'"AND course = "'+subject+'"')
        is_open = cur.fetchall()
        if len(is_open) > 0:
            flash("Attendance for {} is already open".format(subject),category = 'info')
            return redirect(url_for('admin_homepage'))
        else:
            cur = mysql.connection.cursor()
            q1 = 'INSERT INTO att_tracker (id_teacher, teacher_name, status, course, on_time, off_time, date)'
            q2 = 'VALUES("'+str(professor_id)+'","'+professor_name+'","'+status+'","'+subject+'",now(), now(), CURDATE())'
            cur.execute(q1+q2)
            mysql.connection.commit()
            cur.close()
            flash("Attendance turned on successfully for {}".format(subject.replace('_',' ').upper()),category='success')
            return redirect(url_for('admin_homepage'))
...
def turn_off_attendance():
    if request.method == 'POST':
        ...
        check_status = 'open'
        cur = mysql.connection.cursor()
        cur.execute('SELECT * FROM att_tracker WHERE id_teacher = "'+str(professor_id)+'"AND teacher_name = "'+professor_name+'"AND status ="'+check_status+'"AND course = "'+subject+'"')
        is_close = cur.fetchall()
        print(is_close)
        if len(is_close) > 0:
            status = 'close'
            cur = mysql.connection.cursor()
            q1 = 'UPDATE att_tracker SET status = "'+status+'", off_time = now() WHERE id_teacher ="'+str(professor_id)+'" AND course ="'+subject+'"'
            cur.execute(q1)
            mysql.connection.commit()
            cur.close()
            flash("Attendance for {} closed successfully".format(subject.replace('_',' ').upper()),category='success')
            return redirect(url_for('admin_homepage'))
        else:
            flash("Attendance for {} is already closed".format(subject.replace('_',' ').upper()),category = 'info')
            return redirect(url_for('admin_homepage'))
    return render_template('turn_on_attendance.html')
